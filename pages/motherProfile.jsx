...
const MotherProfileScreen = ({ navigation, route }) => {
  ...
  const mother = route?.params?.mother;
  const displayName = mother?.fullName || "Mother";
  const displayId = mother?.nationalId || mother?.id || "Unknown ID";
  const locationParts = [mother?.village, mother?.sector, mother?.district].filter(Boolean);
  const location = locationParts.length > 0 ? locationParts.join(" - ") : "N/A";

  const personalInfo = [
    { label: "Names", value: displayName },
    {
      label: "Age",
      value: mother?.age ? `${mother.age} years` : "N/A",
    },
    { label: "Phone number", value: mother?.phoneNumber || "N/A" },
    { label: "Insurance", value: mother?.insurance || "N/A" },
    { label: "Location", value: location },
  ];

  const pregnancyInfo = [
    { label: "Stage", value: mother?.pregnancyStage || "N/A" },
    { label: "EDD", value: mother?.edd || "N/A" },
    { label: "Health center", value: mother?.healthCenter || "N/A" },
    { label: "Last visit", value: mother?.lastVisitDate || "N/A" },
    { label: "Risk level", value: mother?.riskLevel || "N/A" },
  ];
      ...
      <View style={styles.header}>
        ...
        <View style={styles.profileContainer}>
          ...
          <View style={styles.profileDetails}>
            <Text style={[styles.profileName, styles.fontBold]}>{displayName}</Text>
            <Text style={[styles.profileId, styles.fontRegular]}>{displayId}</Text>
          </View>
        </View>
